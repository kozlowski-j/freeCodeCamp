...
def draw_cat_plot():
    ...
    df_cat = df_cat.groupby(['cardio', 'variable', 'value'], as_index=False)['1'].sum()
    df_cat.rename(columns={'1': 'total'}, inplace=True)
...
def draw_heat_map():
    ...
    sns.heatmap(corr.round(1), annot=True, cmap="RdBu_r", mask=mask, ax=ax, fmt='.1f')
